...
// Commented on 09012023
    public Boolean clickOnHorizantalSubMenu(String menuText) {
        Boolean flag = false;
        try {
            String locator = "Horizantal sub Menu - " + menuText + "#xpath=//div[@class='sub_items']/descendant::a[normalize-space(text())='" + menuText.trim() + "']";
            if (isElementDisplayed(locator)) {
                clickOn(locator);
                waitForPageToLoad();
                flag = true;
    ...
    public void verifyLocatorIsNOT_Displayed(String locator,String comment,String extra) {
        ...
        public void clickOnAnyThingUsingTextAndTagWithForLoop(String tag,String linkText,String extra) {
            try {
                ...
                for (int i = 1; i <= findWebElements(locator).size(); i++) {
                    String button = "" + linkText + " Button#xpath=(//a[normalize-space(text())='" + linkText.trim() + "'])[" + i + "]";
                    if (isElementDisplayed(button)) {
...
//            FileUtils.cleanDirectory(directory);
    ...
    public void navigateBack() {    try {        driver.navigate().back();        waitForPageToLoad();    } catch (Exception e) {        e.printStackTrace();        testStepFailed("Failed in Navigate Back. Exception: "+e.getClass());    }}
