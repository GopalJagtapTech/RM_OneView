...
public class JsonPageLabelsTesting extends ApplicationKeywords {
...
//            String result = new String(Files.readAllBytes(Paths.get("./UploadDocuments/NewHomeAEMTest.json")));
            JSONObject inputJsonObject = new JSONObject(result);
            JSONArray content = inputJsonObject.getJSONArray("content");
//            String remove = content.get(1).toString();
            content.remove(1);
            System.out.println(
...
//
            ...
            group = getKey(input, "group", presentValueGroup, presentValueMap, expectedKeys);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return group;
...
//        getJson("shop-at-emi-store", "laptops-icon", "iconimage-android,iconlink,icontitle");
        getJson("products-for-you","personal-loan","icontitle,iconimage-android,iconlink");
