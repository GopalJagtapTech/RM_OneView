@Bug@Not
Feature: New Home

  Scenario Outline: Shop on Bajaj Mall - <Product Name>
    And Get Data for "<Section Name>" in "Home Page"
    And Get Data for "<Product Name>" under "<Section Name>"
    And verify "<Product Name>" Logo is Displayed under "<Section Name>"
    And verify "<Product Name>" Redirection for "<Section Name>" with New Tab
    And verify Nudge Text with Labels for "<Section Name>" - "<Product Name>"
#    And verify "<Product Name>" Redirection for "<Section Name>" with SSO
    Examples:
      | Section Name       | Product Name |
      | Shop on Bajaj Mall | Smartphones  |


  Scenario Outline: Shop on Bajaj Mall - <Product Name>
    And Get Data for "<Section Name>" in "Home Page"
    And Get Data for "<Product Name>" under "<Section Name>"
    And verify "<Product Name>" Logo is Displayed under "<Section Name>"
    And verify "<Product Name>" Redirection for "<Section Name>" with New Tab
#    And verify "<Product Name>" Redirection for "<Section Name>" with SSO
    Examples:
      | Section Name       | Product Name |
      | Shop on Bajaj Mall | LED TVs      |

  Scenario Outline: Shop on Bajaj Mall - <Product Name>
    And Get Data for "<Section Name>" in "Home Page"
    And Get Data for "<Product Name>" under "<Section Name>"
    And verify "<Product Name>" Logo is Displayed under "<Section Name>"
    And verify "<Product Name>" Redirection for "<Section Name>" with New Tab
#    And verify "<Product Name>" Redirection for "<Section Name>" with SSO
    Examples:
      | Section Name       | Product Name     |
      | Shop on Bajaj Mall | Washing machines |

  Scenario Outline: Shop on Bajaj Mall - <Product Name>
    And Get Data for "<Section Name>" in "Home Page"
    And Get Data for "<Product Name>" under "<Section Name>"
    And verify "<Product Name>" Logo is Displayed under "<Section Name>"
    And verify "<Product Name>" Redirection for "<Section Name>" with New Tab
#    And verify "<Product Name>" Redirection for "<Section Name>" with SSO
    Examples:
      | Section Name       | Product Name  |
      | Shop on Bajaj Mall | Refrigerators |

  Scenario Outline: Shop on Bajaj Mall - <Product Name>
    And Get Data for "<Section Name>" in "Home Page"
    And Get Data for "<Product Name>" under "<Section Name>"
    And verify "<Product Name>" Logo is Displayed under "<Section Name>"
    And verify "<Product Name>" Redirection for "<Section Name>" with New Tab
#    And verify "<Product Name>" Redirection for "<Section Name>" with SSO
    Examples:
      | Section Name       | Product Name |
      | Shop on Bajaj Mall | Smartwatches |

  Scenario Outline: Shop on Bajaj Mall - <Product Name>
    And Get Data for "<Section Name>" in "Home Page"
    And Get Data for "<Product Name>" under "<Section Name>"
    And verify "<Product Name>" Logo is Displayed under "<Section Name>"
    And verify "<Product Name>" Redirection for "<Section Name>" with New Tab
#    And verify "<Product Name>" Redirection for "<Section Name>" with SSO
    Examples:
      | Section Name       | Product Name |
      | Shop on Bajaj Mall | Laptops      |

  Scenario Outline: Shop on Bajaj Mall - <Product Name>
    And Get Data for "<Section Name>" in "Home Page"
    And Get Data for "<Product Name>" under "<Section Name>"
    And verify "<Product Name>" Logo is Displayed under "<Section Name>"
    And verify "<Product Name>" Redirection for "<Section Name>" with New Tab
#    And verify "<Product Name>" Redirection for "<Section Name>" with SSO
    Examples:
      | Section Name       | Product Name |
      | Shop on Bajaj Mall | Cycles       |

  Scenario Outline: Shop on Bajaj Mall - <Product Name>
    And Get Data for "<Section Name>" in "Home Page"
    And Get Data for "<Product Name>" under "<Section Name>"
    And verify "<Product Name>" Logo is Displayed under "<Section Name>"
    And verify "<Product Name>" Redirection for "<Section Name>" with New Tab
#    And verify "<Product Name>" Redirection for "<Section Name>" with SSO
    Examples:
      | Section Name       | Product Name |
      | Shop on Bajaj Mall | Mattresses   |
